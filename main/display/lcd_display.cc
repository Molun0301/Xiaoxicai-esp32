...
SpiLcdDisplay::SpiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    const lvgl_port_display_cfg_t display_cfg = {
        ...
        .double_buffer = false,
        .trans_size = 0,
        .hres = static_cast<uint32_t>(height_),
        .vres = static_cast<uint32_t>(width_),
        .monochrome = false,
        .rotation = {
            .swap_xy = swap_xy,
            .mirror_x = mirror_x,
...
void LcdDisplay::SetupUI() {
    ...
    // 添加黑色圆角矩形背景
    lv_obj_set_style_bg_color(chat_message_label_, lv_color_black(), LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(chat_message_label_, LV_OPA_50, LV_STATE_DEFAULT);
    lv_obj_set_style_radius(chat_message_label_, 10, LV_STATE_DEFAULT);
    lv_obj_set_style_pad_all(chat_message_label_, 8, LV_STATE_DEFAULT);
    lv_obj_align(chat_message_label_, LV_ALIGN_BOTTOM_MID, 0, -50);
...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    // 简化的表情显示，不再执行动作，动作由板级代码统一处理
    lv_obj_set_style_text_font(emotion_label_, fonts_.emoji_font, 0);
    
    // 根据情绪显示对应的表情图标（仅用于状态栏显示）
    if (!strcmp(emotion, "happy") || !strcmp(emotion, "laughing") || !strcmp(emotion, "funny")) {
        lv_label_set_text(emotion_label_, "🙂");
    } else if (!strcmp(emotion, "sad") || !strcmp(emotion, "crying")) {
        lv_label_set_text(emotion_label_, "😔");
    } else if (!strcmp(emotion, "angry")) {
        lv_label_set_text(emotion_label_, "😠");
    } else if (!strcmp(emotion, "surprised") || !strcmp(emotion, "shocked")) {
        lv_label_set_text(emotion_label_, "😯");
    } else if (!strcmp(emotion, "thinking") || !strcmp(emotion, "confused")) {
        lv_label_set_text(emotion_label_, "🤔");
    } else if (!strcmp(emotion, "relaxed") || !strcmp(emotion, "sleepy")) {
        lv_label_set_text(emotion_label_, "😌");
    } else if (!strcmp(emotion, "loving") || !strcmp(emotion, "kissy")) {
        lv_label_set_text(emotion_label_, "😍");
    } else {
